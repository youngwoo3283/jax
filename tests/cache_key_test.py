...
class CacheKeyTest(jtu.JaxTestCase):
  ...
  def test_same_key(self):
    ...
    self.assertEqual(
        cache_key.CacheKey.get(computation, devices, compile_options, backend),
        cache_key.CacheKey.get(computation, devices, compile_options, backend),
    )
  ...
  def test_different_key(self):
    ...
    backend = xla_bridge.get_backend()
    self.assertNotEqual(
        cache_key.CacheKey.get(
            computation, devices, compile_options_not_filled, backend
        ),
        cache_key.CacheKey.get(
            computation, devices, compile_options_filled, backend
        ),
    )
  ...
  def test_different_computations(self):
    ...
    backend = xla_bridge.get_backend()
    self.assertNotEqual(
        cache_key.CacheKey.get(computation1, devices, compile_options, backend),
        cache_key.CacheKey.get(computation2, devices, compile_options, backend),
    )
  ...
  def test_identical_computations_different_metadata(self, include_metadata):
    ...
    with compilation_cache_include_metadata_in_key(include_metadata):
      key1 = cache_key.CacheKey.get(
          computation1, devices, compile_options, backend
      )
      key2 = cache_key.CacheKey.get(
